...
window?.addEventListener("load", async (e) => {
    for (let i = 0; i < medicineNames.length; i++) {
        ...
        if (userDoc.exists()) {
            ...
            document.getElementById(`Quantityy${i + 1}`)?.setAttribute('placeholder', med.quantity-1);
        } else {
...
window?.addEventListener("DOMContentLoaded", async (e) => {
    ...
    if(localStorage['lusername']==="doc1"){
        if (userDoc.exists()) { 
            document.getElementById('name').placeholder = doctor.fullname;
            document.getElementById('dateofbth').value = doctor.dayOfBirth;
            document.getElementById('address').placeholder = doctor.address;
            document.getElementById('gender').placeholder = doctor.gender;
            document.getElementById('occupation').placeholder = doctor.occupation;
            document.getElementById('pnumber').placeholder = doctor.phoneNumber;
            document.getElementById('specialty').placeholder = doctor.specialty;
            document.getElementById('experience').placeholder = doctor.yearsOfExperience;
        } else {
    ...
    if (userDoc.exists()) {
        ...
        document.getElementById('dateofbth')?.setAttribute('placeholder', patient.dayOfBirth);
...
window?.addEventListener("DOMContentLoaded", async (e) => {
        ...
        if (docInfo1 && docInfo2 && docInfo3 && docInfo4) {
            docInfo1.innerHTML = "Name: " + doctor.fullname;
            docInfo2.innerHTML = "Gender: " + doctor.specialty;
